...
Three configurable message types fire whenever a charge is consumed:
...
All three can be toggled independently in `config.yml`.
...
- Creates a fresh Stack Totem with `<amount>` charges and adds it to the
...
*(Silk Touch intentionally removed.)*
...
Key sections in `plugins/ShardShop/config.yml`:
...
|---|---|
| **Vault** | Economy API bridge — lets other plugins read player coin balances |
| **PlaceholderAPI** | `%shardshop_coins%`, `%shardshop_shards%` placeholders |
| **WorldGuard** | Region flags to restrict shop access to specific areas |

None of these are required — ShardShop loads and functions fully without them.

---

*Built for Paper 1.21.4 · Java 21 · ShardShop v1.0.0*
